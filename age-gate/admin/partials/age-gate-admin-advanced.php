...
<div class="wrap">
    ...
    <form class="custom-form-fields" action="admin-post.php" method="post">
        ...
        ($values['endpoint'] === 'rest')
                ...
                <tr class="ep" <?php echo(!$values['use_js'] ? ' style="display: none;"' : '') ?>>
                    <th scope="row"><?php _e('Trap focus', 'age-gate'); ?></th>
                    <td>
                        <label class="ag-switch">
                            <?php echo form_checkbox(
                                array(
                                    'name' => 'ag_settings[trap_focus]',
                                    'id' => 'wp_age_gate_trap_focus'
                                ),
                                1,
                                $values['trap_focus']
                            ); ?><span class="ag-switch__slider"></span>
                        </label> <label><?php echo __('Focus trapping can help accessibility, however may conflict with other popups like cookie banners', 'age-gate') ?></label>
