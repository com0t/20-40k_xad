...
class BeRocket_products_label extends BeRocket_Framework {
    ...
    function __construct () {
        ...
        if ( $this->init_validation() ) {
            ...
            add_action( "elementor/frontend/widget/before_render", array($this, 'activate_style'));
            add_action( "elementor/frontend/widget/after_render", array($this, 'deactivate_style'));
        }

    }
    public $allowedposttags_styleadded = false;
    public function activate_style($instance) {
        if( 'woocommerce-product-related' == $instance->get_name() ) {
            global $allowedposttags;
            if( ! isset($allowedposttags['style']) ) {
                $allowedposttags['style'] = array();
                $allowedposttags['div']['style'] = true;
                $this->allowedposttags_styleadded = true;
                add_action('wp_footer', array($this, 'script_to_fix_styles'));
                add_filter('safe_style_css', array($this, 'safe_style_css'));
            }
        }
    }
    public function deactivate_style($instance) {
        if( 'woocommerce-product-related' == $instance->get_name() ) {
            global $allowedposttags;
            if( $this->allowedposttags_styleadded && isset($allowedposttags['style']) ) {
                unset($allowedposttags['style']);
                $this->allowedposttags_styleadded = false;
            }
        }
    }
    ...
    public function script_to_fix_styles() {
        ?>
        <script>
        jQuery(".br_alabel style").each(function() {
